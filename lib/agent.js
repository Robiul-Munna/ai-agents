...
function getKnowledgeBaseAnswer(question) {
  ...
  // Use simple similarity: count matching words, pick highest
  let bestMatch = null;
  let bestScore = 0;
  for (const item of allKnowledge) {
    ...
    const itemWords = itemQ.split(/\W+/).filter(Boolean);
  ...
  if (bestMatch && bestScore >= Math.ceil(q.split(/\W+/).filter(Boolean).length / 2)) {
