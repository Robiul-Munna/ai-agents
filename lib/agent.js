...
// Move getKnowledgeBaseAnswer to top-level scope
function getKnowledgeBaseAnswer(question) {
  // Debug: log knowledge base length and Parkinson's entry
  console.log(`[KB Debug] generalKnowledgeBase length: ${generalKnowledgeBase.length}`);
  console.log(`[KB Debug] advancedKnowledgeBase length: ${advancedKnowledgeBase.length}`);
  const parkinsonsEntry = [...generalKnowledgeBase, ...advancedKnowledgeBase].find(item => item.question.toLowerCase().includes("parkinson"));
  if (parkinsonsEntry) {
    console.log(`[KB Debug] Parkinson's entry found: ${JSON.stringify(parkinsonsEntry)}`);
  } else {
    console.log(`[KB Debug] Parkinson's entry NOT found`);
  }
  ...
  for (const item of allKnowledge) {
    ...
    // Force exact match for Parkinson's question
    if (q.trim() === "how to manage parkinson's disease symptoms?") {
      if (item.question.toLowerCase().includes("parkinson")) {
        console.log(`[KB Forced Parkinson's Match] Q: '${question}' | KB: '${item.question}'`);
